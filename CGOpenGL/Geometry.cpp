...
Geometry::Geometry( const std::string& objFilename )
{
	...
	for( tinyobj::material_t mat : materials )
	{
		...
		Scene::RegisterMaterial( objFilename + mat.name, m );
	}
	...
	for( tinyobj::shape_t shape : shapes )
	{
		...
		// Init the mesh
		if( index && tex )
			m->Initialize( VertexFormatManager::Get3F3F2F(), &mem[0], mem.size() * 4, shape.mesh.indices, GL_TRIANGLES, mat );
		else if( index )
			m->Initialize( VertexFormatManager::Get3F3F(), &mem[0], mem.size() * 4, shape.mesh.indices, GL_TRIANGLES, mat );
		else if( tex )
			m->Initialize( VertexFormatManager::Get3F3F2F(), &mem[0], mem.size() * 4, GL_TRIANGLES, mat );
		else
			m->Initialize( VertexFormatManager::Get3F3F(), &mem[0], mem.size() * 4, GL_TRIANGLES, mat );

		meshes.push_back( m );
		Scene::RegisterMesh( objFilename + shape.name, m );
	}
}
